...
def sammleAlleKursLinks(bereiche: list[str]):
    ...
    with open("afae-alle-kurse.txt", 'w+') as wf:
        wf.writelines(f"{kurs}\n" for kurs in alle_kurs_links)
    wf.close()

def scrapeHtml(kurs_link: str, kurs_nummer: str):
    try:
        response = r.get(kurs_link, timeout=10)
        html = bs(response.text, 'html.parser')
        tds = html.find_all("td")
        data = {"Kurs-Id": kurs_nummer, "Kursname": kurs_link.split("/")[-1]}
...
def findeKursDetailsMitKursNummer(kurs_nummer: str):
    kurs_link = ""
    with open("afae-alle-kurse.txt", 'r') as rf:
        kurse = rf.readlines()
        for kurs in kurse:
